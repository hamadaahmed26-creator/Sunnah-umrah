...
## Session — App Store Launch Setup (no Mac) — current session
- **`/app/frontend/ios/`** (NEW) — Capacitor 7 iOS native project scaffolded via
  `npx cap add ios`. Bundle ID `com.sunnahumrah.app`. Pod install will run on
  Codemagic's Mac at build time (CocoaPods not available on this Linux container).
- **iOS App Icons + Splash** — generated by `@capacitor/assets` from
  `/app/frontend/resources/icon.png` & `splash.png`. 1024 base icon + light/dark
  splash at 1x/2x/3x universal sizes (10 files, 4.47 MB total).
- **`/app/frontend/ios/App/App/Info.plist`** patched with:
  - `NSLocationWhenInUseUsageDescription` + `NSLocationAlwaysAndWhenInUseUsageDescription`
    explaining the "I'm lost" gate-finder feature
  - `ITSAppUsesNonExemptEncryption = false` (skips export compliance form on every upload)
  - `LSApplicationQueriesSchemes` for `comgooglemaps`, `maps`, `tel`
  - Custom URL scheme `sunnahumrah://`
- **`/app/codemagic.yaml` (NEW)** — full CI/CD pipeline for Codemagic cloud Mac:
  yarn install → yarn build → cap sync ios → pod install → auto-bump build number
  → xcode-project build-ipa → upload to TestFlight via App Store Connect API.
  Submit-to-App-Store toggle is FALSE on first build (user fills metadata first),
  flipped TRUE after.
- **`/app/APP_STORE_LAUNCH_NO_MAC.md` (NEW)** — kid-level step-by-step guide
  replacing the old Mac-dependent flow. Covers: ASC app record creation,
  ASC API key generation, Codemagic team setup, first build trigger,
  privacy/support URL hosting, App Privacy questionnaire (Travelpayouts marker
  is NOT IDFA tracking), screenshot generation, and anti-rejection checklist.

## Pending from user (to finish App Store launch)
- Numeric **App Store App ID** (created in App Store Connect → copy from URL)
- Confirmation email for Codemagic build notifications
- Whether to auto-generate /privacy + /support pages in the React app
- Whether to auto-generate App Store screenshots via Playwright (1290×2796)
